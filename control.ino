...
const int DAC8831FullValue = 65536;
const float DAC8831RefVolt = 2.5;
...
void SPIDigitalPortWrite(int value){
	digitalWrite(slaveSelectPin, LOW);
	SPI.transfer(value);
	digitalWrite(slaveSelectPin, HIGH);
	delay(100);//写完以后等待100ms
}
...
//中断 监听中断关机中断
void interruptSetup(){
	attachInterrupt(pbIn,shutdownProgram,LOW);
}
...
void shutdownProgram(){
	...
	SPIDigitalPortWrite(DAC8831FullValue / 2);
	delay(200);
...
bool findPeakValue(){
	for (int i = 0; i < maxLen; i++){
		if (ErrSig[i] == ADFullValue / 2){//满足为0点情况下
			if (i == 0 || i == maxLen - 1)
				break;
			if ((ErrSig[i] > ErrSig[i - 1] && ErrSig[i] < ErrSig[i + 1])
				|| (ErrSig[i] < ErrSig[i - 1] && ErrSig[i] > ErrSig[i + 1])){
				return true;
			}
		}
	}
	return false;//如果整个循环完成都没有找到峰值，那么输出false
}
...
void work(){
	...
	int PWMVoltOut = DAC8831FullValue / 2;//设定从0开始++,0对于DAC8831的值为 DAC8831FullValue / 2；
	float volt = PWMSetValue / 40.0 * DAC8831FullValue / 2 / DAC8831RefVolt + DAC8831FullValue / 2;; //
	while (PWMVoltOut < volt){
		PWMVoltOut++; 
		SPIDigitalPortWrite(PWMVoltOut);//SPI输出电压值
	}
	...
	if(ad2_value != volt){ //如果和设定值90mA对应电压值不同，报错
		...
		while(1){
			...
			index++;
			PWMVoltOut++;
			SPIDigitalPortWrite(PWMVoltOut);//该地方是都考虑要使用spi输出该值
		...
		bool ifFindPeak = findPeakValue();
		if( ifFindPeak == false){
			showError();
			shutdownProgram();
		}
		else{
			int peakVolt = 2.5 * DAC8831FullValue / 2 / DAC8831RefVolt + DAC8831FullValue / 2;
			SPIDigitalPortWrite(peakVolt);
